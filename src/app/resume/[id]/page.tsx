...
export default function ResumeEditor() {
  ...
  const { register, handleSubmit, reset, watch, setValue, control } = useForm<FormValues>({
    defaultValues: {
      title: "",
      personalInfo: { fullName: "", email: "", phone: "", location: "", github: "", portfolio: "" },
      summary: "",
      experience: [],
      education: [],
      projects: [],
    ...
    const fetchResume = async () => {
      try {
        ...
        const formattedData = {
          ...
          projects: data.projects?.map((p: any) => ({ ...p, techStack: p.techStack?.join(", ") || "" })) || []
        };
        reset(formattedData);
      } catch (error) {
  ...
  const onSubmit = async (formData: FormValues) => {
    ...
    try {
      await axios.put(`/api/resume/${params.id}`, payload);
